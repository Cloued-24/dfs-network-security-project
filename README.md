...
| `show crypto isakmp sa` | IPSec Phase 1 status (`QM_IDLE` = healthy) |
...
| `show crypto isakmp sa` shows `QM_IDLE` | Tunnel Phase 1 active | ✅ Confirmed |
...
| HR couldn't ping servers despite tunnel showing `QM_IDLE` | DNS/WEB/EMAIL servers had never been assigned IP addresses | Assigned static IP/gateway to each server |
